...
public:
    ...
    void share_combine(mpz_class &rop, vector<mpz_t> &shares) {
        pcs_t_share_combine(vk, rop.get_mpz_t(), shares.data());
    }
...
{
    pcs_t_generate_key_pair(pk.as_ptr(), vk.as_ptr(), vk.get_rand(), bits,
            w, l);
}
...
public:
    polynomial(hcs::rand &hr_) {
        pk = pcs_t_init_polynomial();
        hr = &hr_;
        hr->inc_refcount();
    }

    ~public_key() {
        pcs_t_free_polynomial(pk);
        hr->dec_refcount();
    }
    ...
    void compute(pcs_t::private_key &vk, mpz_class &rop, const unsigned long x) {
        pcs_t_compute_polynomial(vk.as_ptr(), p, rop.get_mpz_t(), x);
    }
};
